# Verita — Layout Components

## AppShell — `frontend/src/components/AppShell.tsx`

Wraps all inner pages. Provides a sticky sidebar + top header + scrollable main area.
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg)" }}>
      ...
      >
        ...
        {/* Nav items — active: surface-2 bg + blue left inset shadow */}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        {/* Header — 64px, page title + theme toggle + avatar */}
        <header
          style={{
            height: 64,
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button data-cursor onClick={toggle} style={{ width: 38, height: 38, borderRadius: 10, border: "1px solid var(--border)", background: "var(--surface)", color: "var(--text)", display: "grid", placeItems: "center" }}>
              {theme === "dark" ? <Sun size={17} /> : <Moon size={17} />}
            </button>
            {/* Avatar — signature gradient circle */}
...
`/` renders `ImmersiveLanding` — fullscreen, no sidebar, no header. Custom cursor disabled.
